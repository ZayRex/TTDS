...
class PREPROCESS:
    ...
    def process_tsv(tsv_file, stopwords):
        ...
        corpora_tokens_counts = defaultdict(lambda: defaultdict(int))
        with open(tsv_file, 'r') as data:
            ...
            for row in csv_reader:
                ...
                for t in set(row_tokens):
                    corpora_tokens_counts[row[0]][t] += 1
        #number of all documents n
        n = sum(len(c) for c in docs_tokens.values())
        return docs_tokens, corpora_tokens_counts, n
...
class ANALYSIS:
    ...
    def analyse_corpora(docs_tokens, corpora_tokens_counts, n):
        mutual_info, chi_squared = {}, {}
        for corpus, tokens in corpora_tokens_counts.items():
            mutual_info[corpus] = {}
            chi_squared[corpus] = {}
            for token, count in tokens.items():
                ...
                n10 = sum([tokens[token] for c, tokens in corpora_tokens_counts.items() if c != corpus])
...
class CLASSIFY:
    def split_train_dev(verses, corpora, train_split, dev_split):        
        X_train, X_dev, y_train, y_dev = sklearn.model_selection.train_test_split(verses, corpora, train_size=train_split, test_size=dev_split, random_state=77)
    ...
    def prepare_baseline_preprocessed_data(verses, corpora, vocab):
        X_train, nX_dev, y_train, y_dev = CLASSIFY.split_train_dev(verses, corpora, 0.7, 0.3)
        word2id, corpora2id = CLASSIFY.word2id(vocab)

        X_train = CLASSIFY.convert_to_bow_matrix(X_train, word2id)
        X_dev = CLASSIFY.convert_to_bow_matrix(nX_dev, word2id)
        y_train = [corpora2id[c] for c in y_train]
        y_dev = [corpora2id[c] for c in y_dev]
        return X_train, X_dev, y_train, y_dev, nX_dev

    def prepare_improved_preprocessed_data(verses, corpora, vocab):
        X_train, X_dev, y_train, y_dev = CLASSIFY.split_train_dev(verses, corpora, 0.9, 0.1)
        word2id, corpora2id = CLASSIFY.word2id(vocab)
    ...
    def prepare_baseline_test_data(verses, corpora, vocab):
        
        word2id, corpora2id = CLASSIFY.word2id(vocab)
        X_test = CLASSIFY.convert_to_bow_matrix(verses, word2id)
        y_test = [corpora2id[c] for c in corpora]
        return X_test, y_test,
    
    def prepare_improved_test_data(verses, corpora, vocab):
    ...
    def train_base_model(data, test_data):
        verses, corpora, vocab = CLASSIFY.baseline_data_preprocess(data)
        X_train, X_dev, y_train, y_dev, nX_dev = CLASSIFY.prepare_baseline_preprocessed_data(verses, corpora, vocab)
        ...
        test_verses, test_corpora, _ = CLASSIFY.baseline_data_preprocess(test_data)
        X_test, y_test = CLASSIFY.prepare_baseline_test_data(test_verses, test_corpora, vocab)
    ...
    def train_improved_model(data, test_data):
        verses, corpora, vocab = CLASSIFY.improved_data_preprocess(data)
        X_train, X_dev, y_train, y_dev = CLASSIFY.prepare_improved_preprocessed_data(verses, corpora, vocab)
        ...
        test_verses, test_corpora, _ = CLASSIFY.improved_data_preprocess(test_data)
        X_test, y_test = CLASSIFY.prepare_improved_test_data(test_verses, test_corpora, vocab)
...
if __name__ == "__main__":
    ...
    docs_tokens, corpora_tokens_counts, n = PREPROCESS.process_tsv(tsv_file, stopwords)

    mi, chi_sq = ANALYSIS.analyse_corpora(docs_tokens, corpora_tokens_counts, n)
